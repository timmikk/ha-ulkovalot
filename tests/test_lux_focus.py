...
def test_lux_storm_ceiling_lock_holds():
    ...
    for lux in (500, 300, 100, 50, 10):
        dark = is_dark(elev=20, lux=lux, last_dark=last_dark, cfg=CFG)
...
def test_lux_dawn_boost_flips_phase_to_day():
    ...
    dark_before = is_dark(elev, lux=20, last_dark=True, cfg=CFG)
    phase_before = derive_phase(now, rising, dark_before, CFG)
    assert dark_before is True
    assert phase_before is Phase.MORNING

    dark_after = is_dark(elev, lux=CFG.lux_off_above, last_dark=dark_before, cfg=CFG)
...
def test_sensor_lost_falls_back_to_sun_only():
    ...
    dark_with_sensor = is_dark(elev, lux=150, last_dark=True, cfg=CFG)
    ...
    dark_fallback = is_dark(elev, lux=lux, last_dark=dark_with_sensor, cfg=CFG)
...
def test_floor_lock_ignores_torch_on_sensor():
    ...
    dark = is_dark(elev, lux=200, last_dark=True, cfg=CFG)
...
def test_multi_sensor_median_ignores_covered_sensor():
    ...
    dark = is_dark(elev=0, lux=lux, last_dark=True, cfg=CFG)
    assert dark is False
