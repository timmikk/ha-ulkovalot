...
- [ ] `async_setup_entry`: `_LOGGER.debug("Setting up entry %s", entry.entry_id)` before starting the coordinator, `_LOGGER.info("ulkovalot entry %s set up", entry.entry_id)` after
- [ ] `async_unload_entry`: `_LOGGER.debug("Unloading entry %s", entry.entry_id)` before `coordinator.unload()`
- [ ] `_handle_override`: `_LOGGER.info("Service override called: scene=%s duration=%s", call.data.get("scene"), call.data.get("duration"))`
- [ ] `_handle_cancel`: `_LOGGER.info("Service cancel_override called")`
...
- [ ] `start_override`: `_LOGGER.info("Override started: scene=%s until=%s", self.override_scene, self.override_until)` after setting `self.override_until`
- [ ] `cancel_override`: `_LOGGER.info("Override cancelled")` before `apply_scene()`
- [ ] `_on_expiry`: `_LOGGER.info("Override expired")` before `_clear_override_state()`
- [ ] `wire_trigger`: `_LOGGER.debug("Subscribed override trigger: %s", trigger)` when a trigger entity is configured
...
- [ ] Early-return when `disable_flag` is on: `_LOGGER.debug("Disabled via %s — skipping apply", cfg.disable_flag)`
- [ ] After computing `phase`, `motion`, `override`, `scene_key`: `_LOGGER.debug("Apply: elev=%.1f lux=%s dark=%s phase=%s motion=%s override=%s -> %s", elev, lux, self.last_dark, phase, motion, override, scene_key)`
- [ ] Right before `async_call(SCENE_DOMAIN, ...)`: `_LOGGER.debug("Dispatching scene.turn_on entity=%s transition=%s", entity, transition)`
...
- [ ] `_on_sun_state`: when `_crossed(...)` is true, `_LOGGER.debug("Sun elevation crossed threshold: %.1f -> %.1f", old_elev, new_elev)` before `self._schedule_apply()`
- [ ] `_on_watched_state`: `_LOGGER.debug("Watched entity changed: %s -> %s", entity_id, new.state if new else None)` before `self._schedule_apply()`
...
- [ ] `tests/test_init.py` — new test asserting `async_setup_entry` logs an INFO "set up" record containing the entry id; new test asserting the `override` and `cancel_override` service handlers each log an INFO record when called
- [ ] `tests/test_coordinator.py` — new test asserting `start_override` logs INFO containing the resolved scene; new test asserting `cancel_override` logs INFO; new test asserting `_on_expiry` logs INFO "expired"; new test (DEBUG level) asserting `_apply_scene_impl` logs the decision summary line with the resolved `scene_key`; new test asserting the disable-flag short-circuit logs DEBUG and skips dispatch
...
- [ ] Rollout / state invalidation reflected (n/a, noted above)
- [ ] Tests in "Test updates" added/updated
- [ ] `logger: custom_components.ulkovalot: debug` in a test HA config surfaces a full decision trace for one apply cycle (spot-checked manually or via the new tests' captured records)
